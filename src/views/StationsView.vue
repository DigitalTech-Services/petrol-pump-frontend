  ...
  <div>
    ...
    <div class="card">
      <div class="card-header">
        <div>
          <div class="font-display font-bold text-[15px] text-white">Stations</div>
          <div class="text-[11.5px] text-[#5a6a82] mt-0.5">Each station is run by one manager at a time</div>
        </div>
      </div>

      <div v-if="store.loading" class="p-8 text-center text-[#5a6a82] text-[13px]">Loading stations…</div>
        ...
        <table class="data-table">
          ...
          <tbody>
            <tr v-if="store.records.length === 0">
              <td colspan="6" class="text-center text-[#5a6a82] py-6 text-[13px]">No stations yet. Add the first station.</td>
            </tr>
            <tr v-for="(s, i) in store.records" :key="s.id">
              ...
              <td class="text-[#8a9ab5]">{{ s.city || '—' }}</td>
              <td class="font-mono-custom text-[12px]">{{ s.gst || '—' }}</td>
              <td>
                <span v-if="s.manager" class="text-[#8a9ab5]">{{ s.manager.name }}</span>
